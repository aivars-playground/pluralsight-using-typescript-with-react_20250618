"use client";
import { MouseEvent, useReducer, useTransition } from "react";
...
interface SpeakerFavoriteToggleProps {
  speakerRec: Speaker;
}
...
function speakerReducer(state: State, action: Action): State {
  switch (action.type) {
...
export default function SpeakerFavoriteToggle({
  speakerRec,
}: SpeakerFavoriteToggleProps) {
  const initialState: State = { speaker: speakerRec, error: null };
  ...
  async function handleToggleFavorite(e: MouseEvent<HTMLButtonElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        const validatedSpeaker = speakerSchema.parse(updatedSpeaker);

        await speakerFavoriteToggleAction(validatedSpeaker);
      } catch (err: unknown) {
